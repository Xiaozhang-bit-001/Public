import os
import random
import numpy as np
import torch
from scipy import ndimage
from scipy.ndimage import zoom
from torch.utils.data import Dataset


# -------------------------- 核心修复：数据增强函数（确保通道维度不被打乱） --------------------------
def random_rot_flip(image, label):
    # 1. 先将 (C, H, W) 转成 (H, W, C)（通道放最后，避免空间操作影响通道）
    image = image.transpose(1, 2, 0)  # (C,H,W) → (H,W,C)
    # 2. 执行旋转翻转（此时只作用于H/W维度，不影响通道）
    k = np.random.randint(0, 4)
    image = np.rot90(image, k)
    label = np.rot90(label, k)
    axis = np.random.randint(0, 2)  # 0=水平翻转，1=垂直翻转（只作用于H/W）
    image = np.flip(image, axis=axis).copy()
    label = np.flip(label, axis=axis).copy()
    # 3. 转回 (C, H, W) 格式
    image = image.transpose(2, 0, 1)  # (H,W,C) → (C,H,W)
    return image, label


def random_rotate(image, label):
    # 1. 先将 (C, H, W) 转成 (H, W, C)
    image = image.transpose(1, 2, 0)
    # 2. 执行旋转（只作用于H/W维度）
    angle = np.random.randint(-20, 20)
    image = ndimage.rotate(image, angle, order=3, reshape=False)
    label = ndimage.rotate(label, angle, order=0, reshape=False)
    # 3. 转回 (C, H, W) 格式
    image = image.transpose(2, 0, 1)
    return image, label


# -------------------------- 其余代码不变 --------------------------
class RandomGenerator(object):
    def __init__(self, output_size=(256, 256), mean=None, std=None):
        self.output_size = output_size
        assert mean is not None and std is not None, "必须传入均值mean和标准差std"
        assert len(mean) == 3 and len(std) == 3, "均值和标准差必须为3通道"
        self.mean = mean
        self.std = std

    def __call__(self, sample):
        image, label, case_name = sample['image'], sample['label'], sample['case_name']
        # print(f"[{case_name}] 初始维度：image={image.shape}, label={label.shape}")

        # 第一步：强制统一维度为 (C, H, W)
        if image.ndim == 3:
            if image.shape[-1] == 3:
                image = image.transpose(2, 0, 1)  # (H,W,C)→(C,H,W)
                # print(f"[{case_name}] 维度转换：(H,W,C)→{image.shape}")
            elif image.shape[1] == 3:
                image = image.transpose(1, 0, 2)  # (H,C,W)→(C,H,W)
                # print(f"[{case_name}] 维度转换：(H,C,W)→{image.shape}")
            elif image.shape[0] == 3:
                # print(f"[{case_name}] 维度正常：{image.shape}")
                a=1
            else:
                raise ValueError(f"[{case_name}] 3D图像无3通道：{image.shape}")
        else:
            raise ValueError(f"[{case_name}] 非3D图像：{image.ndim}D，形状{image.shape}")

        assert image.shape[0] == 3, f"[{case_name}] 统一后通道数≠3：{image.shape}"
        # print(f"[{case_name}] 统一后维度：{image.shape}")

        # 第二步：数据增强（修复后不会改变维度顺序）
        orig_image_shape = image.shape
        if random.random() > 0.5:
            image, label = random_rot_flip(image, label)
            # print(f"[{case_name}] 旋转翻转后：image={image.shape}, label={label.shape}")
        if random.random() > 0.5:
            image, label = random_rotate(image, label)
            # print(f"[{case_name}] 随机旋转后：image={image.shape}, label={label.shape}")

        # 断言会通过（增强后维度不变）
        assert image.shape == orig_image_shape, f"[{case_name}] 增强后维度改变：{orig_image_shape}→{image.shape}"

        # 第三步：强制缩放（通道维度不缩放）
        C, H, W = image.shape
        target_H, target_W = self.output_size
        img_zoom_ratio = (1.0, target_H / H, target_W / W)
        # print(f"[{case_name}] 缩放前：{image.shape}，缩放比={img_zoom_ratio}")

        image = zoom(image, img_zoom_ratio, order=3)
        label = zoom(label, (target_H / H, target_W / W), order=0)

        assert image.shape[0] == 3, f"[{case_name}] 缩放后通道数≠3：{image.shape}"
        assert image.shape[1:] == (target_H, target_W), f"[{case_name}] 缩放后尺寸异常：{image.shape[1:]}≠{self.output_size}"
        # print(f"[{case_name}] 缩放后维度：image={image.shape}, label={label.shape}")

        # 第四步：归一化
        for c in range(3):
            image[c, :, :] = (image[c, :, :] - self.mean[c]) / self.std[c]

        # 最终Tensor维度
        image_tensor = torch.from_numpy(image.astype(np.float32))
        label_tensor = torch.from_numpy(label.astype(np.int64))
        # print(f"[{case_name}] 最终Tensor维度：image={image_tensor.shape}, label={label_tensor.shape}\n")

        return {
            'image': image_tensor,
            'label': label_tensor,
            'case_name': case_name
        }


class Synapse_dataset(Dataset):
    def __init__(self, base_dir, list_dir, split, transform=None):
        self.transform = transform
        self.split = split
        self.sample_list = [line.strip() for line in open(os.path.join(list_dir, f"{self.split}.txt")).readlines()]
        self.data_dir = base_dir

    def __len__(self):
        return len(self.sample_list)

    def __getitem__(self, idx):
        file_name = self.sample_list[idx]
        data_path = os.path.join(self.data_dir, file_name)

        try:
            data = np.load(data_path)
            image, label = data['image'], data['label']
            # print(f"[{file_name}] 加载原始数据：image={image.shape}, label={label.shape}")
        except Exception as e:
            raise RuntimeError(f"[{file_name}] 加载失败：{str(e)}")

        # 标签清洗
        num_classes = 6
        label = np.where(label == 255, 0, label)
        label = np.clip(label, 0, num_classes - 1)

        sample = {'image': image, 'label': label, 'case_name': file_name}

        if self.transform:
            sample = self.transform(sample)

        return sample