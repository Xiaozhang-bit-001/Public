...
# -------------------------- 主测试函数 --------------------------
def main():
    parser = argparse.ArgumentParser(description='遥感图像分割测试（三图可视化：原图+标签+预测）')
    parser.add_argument('--model_name', type=str, default='SegViTRS',
                        choices=['DANet', 'SegViTRS',
    ...
    if   args.model_name == 'DANet':
        net = DANet(num_classes=args.num_classes,in_channels=3).to(device)
    ...
    # ====== 核心修复：TransUNet 权重名称“翻译器” ======
    if args.model_name == 'TransUNet':
        mapped_weights = {}
        for k, v in model_weights.items():
            # 1. 翻译 feature_projection (旧独立层 -> 新Sequential的第0层)
            ...
            # 2. 翻译 proj_norm (旧独立BN层 -> 新Sequential的第1层)
            ...
            # 3. 过滤掉新代码已经不需要的 decoder 多余 bias
    ...
    # 使用 strict=False 加载（因为我们手动丢弃了没用的 bias）
