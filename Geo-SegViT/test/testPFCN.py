...
# 模型配置
MODEL_NAME = 'FCN'
MODEL_WEIGHT_PATH = r"/root/autodl-tmp/Geo-SegViT/ComparedModels_U/FCN_P/best.pth"
...
# 输出配置
OUTPUT_DIR = r'/root/autodl-tmp/Geo-SegViT/TestResults/FCN_P'
...
# ======================== 2. 数据加载 ========================
class FCNTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            img_trans = img.transpose(1, 2, 0).astype(np.float32)  # ✅ 移除 /255
...
# ======================== 4. 可视化函数 ========================
def vis_fcn_result(img, mask, pred, file_name, save_path, mean, std):
    img_np = img.cpu().numpy().transpose(1, 2, 0)
    img_np = img_np * std + mean
    img_np = np.clip(img_np, 0, 255).astype(np.uint8)  # ✅ 修复
...
class FCN(nn.Module):
    def __init__(self, num_classes=6, in_channels=3):
        ...
        self.fcn = nn.Conv2d(2048, num_classes, kernel_size=1, bias=True)
...
def main():
    ...
    metrics = FCNMetrics(NUM_CLASSES)
    vis_count = 0
