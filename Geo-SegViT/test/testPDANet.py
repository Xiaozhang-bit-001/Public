...
MODEL_WEIGHT_PATH = r"G:\erya308\zhangjunming\Geo-SegViT\ComparedModels\FinalResultsP\DANet\Pots_256\DANet_Pots_256_256_bs8_ep100_final_20251210_180919.pth"
CONFIG_PATH = r"G:\erya308\zhangjunming\Geo-SegViT\ComparedModels\FinalResultsP\DANet\Pots_256\DANet_Pots_256_256_bs8_ep100_config_20251210_180919.json"
...
OUTPUT_DIR = r'G:\erya308\zhangjunming\Geo-SegViT\ComparedModels\TestResults\DANet_P'
...
# ======================== 2. 数据加载（修复numpy()错误） ========================
class DANetTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 调试：验证数据范围（修复核心错误：mask已是NumPy数组，无需调用.numpy()）
        if DEBUG and idx == 0:
            unique_labels = np.unique(mask)  # 移除 .numpy()
...
# ======================== 4. 可视化（同步修复反归一化） ========================
def vis_danet_result(img, mask, pred, file_name, save_path, mean, std):
    # 反归一化（还原到原始像素值）
    img_np = img.cpu().numpy().transpose(1, 2, 0)  # (3,H,W) → (H,W,3)
    img_np = img_np * std + mean
    img_np = np.clip(img_np, 0, 255).astype(np.uint8)  # 移除重复的 *255

    # 标签转彩色图（mask/pred是Tensor，需先转NumPy）
    ...
    axes[2].set_title('DANet预测结果', fontsize=12)
    axes[2].axis('off')
...
def main():
    ...
    # 验证类别配置
    train_class_names = train_config.get('class_names', [])
    if train_class_names and train_class_names != CLASS_NAMES:
        print(f"⚠️ 类别配置不匹配！")
        print(f"训练配置类别: {train_class_names}")
        print(f"测试脚本类别: {CLASS_NAMES}")

    # 数据预处理（修复归一化）
    def transform(img):
        """修正：均值/标准差是原始像素值，无需除以255"""
        img = img.astype(np.float32)
        img = (img - train_mean) / train_std
        return img

    # 加载数据集
    test_dataset = DANetTestDataset(
        root_path=ROOT_PATH,
        img_size=IMG_SIZE,
        transform=transform
    )
    ...
    if output_std < 1e-3:
        print("❌ 模型权重异常！输出无方差，可能是权重未训练或加载错误")
    else:
        print("✅ 模型权重验证通过")

    # 初始化指标
    metrics = DANetMetrics(NUM_CLASSES)
    metrics.reset()
    ...
    vis_count = 0
    with torch.no_grad():
        for batch_idx, (imgs, masks, file_names) in enumerate(tqdm(test_loader, desc='DANet测试进度')):
            imgs = imgs.to(DEVICE)
    ...
    # 保存结果
    metrics_path = os.path.join(test_output_dir, 'danet_test_metrics.json')
...
"""
    ...
    report_path = os.path.join(test_output_dir, 'danet_test_report.md')
