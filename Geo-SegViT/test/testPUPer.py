...
class UperNetTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = data['image']  # 原始0-255 uint8图像 (3, 256, 256)
        mask = data['label']
        ...
        if DEBUG and idx == 0:
            ...
            print(f"🔍 原始图像像素范围: [{img.min().item():.2f}, {img.max().item():.2f}]")
...
def vis_upernet_result(img, mask, pred, file_name, save_path, mean, std):
    ...
    axes[2].set_title('UperNet Pred', fontsize=12)
    axes[2].axis('off')
...
def main():
    ...
    train_std = stats['std']
    print(f"📊 均值(0-255): {train_mean.round(4)}")
    print(f"📊 标准差(0-255): {train_std.round(4)}")

    # ✅ 【唯一正确的归一化】和训练完全一致，不除以255
    def transform(img):
    ...
    print(f"输入尺寸: {test_input.shape}, 输出尺寸: {test_output.shape}")
    assert test_output.shape == (1, 6, 256, 256), "模型输出尺寸不匹配！"
    print("✅ 模型输出验证通过")
